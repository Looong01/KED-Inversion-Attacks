...
def unfreeze(net):
    '''
    Unfreeze the network during training
    '''
    for p in net.parameters():
...
'''
save_img_dir = "./Checkpoints/new_celeba_gan/improvedGAN/imgs_improved_celeba_gan"
save_model_dir = "./Checkpoints/new_celeba_gan/improvedGAN/"
os.makedirs(save_model_dir, exist_ok=True)
os.makedirs(save_img_dir, exist_ok=True)

dataset_name = "celeba"

log_path = "./Checkpoints/new_celeba_gan/improvedGAN/attack_logs"
...
if __name__ == "__main__":
    ...
    for i in range(1, 4):
        if i == 1:
            ...
            batch_size = 64*4
        with open(os.path.join(save_model_dir, 'k+1_gan.log'), 'a') as f:
        ...
        for j in range(1, 4):
            ...
            if model_name_T.startswith("VGG16"):
                T = VGG16(1000)
                path_T = './Checkpoints/Target_Model_Checkpoints/VGG16_88.26.tar'
            elif model_name_T.startswith('IR152'):
                T = IR152(1000)
                path_T = './Checkpoints/Target_Model_Checkpoints/IR152_91.16.tar'
            elif model_name_T == "FaceNet64":
                T = FaceNet64(1000)
                path_T = './Checkpoints/Target_Model_Checkpoints/FaceNet64_88.50.tar'
            ...
            for epoch in range(epochs):
                ...
                for i, imgs in enumerate(dataloader):
                    ...
                    imgs = imgs.cuda()
                    bs = imgs.size(0)
                    x_unlabel = unlabel_loader1.__next__()
                    x_unlabel2 = unlabel_loader2.__next__()
                ...
                with open(os.path.join(save_model_dir, 'k+1_gan.log'), 'a') as f:
                    f.write("Epoch:%d \tTime:%.2f\tG_loss:%.2f\t train_acc:%.2f\n" % (epoch, interval, g_loss, acc))
