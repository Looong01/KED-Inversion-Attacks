...
device = "cuda"
num_classes = 1000
save_img_dir = './recovery/res_all'  # all attack imgs
os.makedirs(save_img_dir, exist_ok=True)
success_dir = './recovery/res_success'
...
def dist_inversion(G, D, T, E, iden, itr, lr=2e-2, momentum=0.9, lamda=100, iter_times=1500, clip_range=1, improved=False, num_seeds=5):
    ...
    1. We use the same code for both the baseline and the improved version. For the baseline version, we simply set the
    input of the discriminator as the image generated by the generator. For the improved version, we add a softmax layer
    for the output of the discriminator.
    
    2. Since the output of the discriminator is 1, we use the cross-entropy loss in the baseline version, and the
    negative mean in the improved version.
    
    3. For the improved version, we add a prior of the output of the discriminator and the label of the input image.
    This is to make the discriminator have the ability to classify the image into the right class.
    
    4. For the improved version, we use the softplus function to calculate the log sum exponential of the output of the
    discriminator. This is to avoid the problem of gradient explosion. 
    
    5. We use distributional inversion to attack the target model. The main idea is to use the prior of the output of
    """
