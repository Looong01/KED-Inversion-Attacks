...
def get_center_mask(img_size, bs):
    ...
    * inputs:
        ...
        - bs (int, required)
                Batch size.
    * returns:
        A torch tensor of shape (bs, 1, img_size, img_size) filled with 1.0 except for the center area.
    '''
...
def low2high(img):
    '''
    Convert the low resolution image to high resolution image.
    '''
    # 0 and 1, 64 to 112
    bs = img.size(0)
